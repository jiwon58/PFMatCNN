    ...
    def featurelist(self):
        ...
        for i in range(5):
            for j in range(len(self.conv[i])):
                features.append(self.conv[i][j])
            features.append(self.pool[i])
        for i in range(5):
            features.append(self.unpool[i])
            for j in range(len(list(self.conv[i+5]))):
                features.extend(self.conv[i+5][j])
        #features.append(self.linear)
